...
export class expr {
    ...
    get length() {
        ...
        let len = 0;
        for (let tok = this.front; this.end(tok); tok = this.next(tok))
            ++len;
        return len;
    };
     ...
     */
    empty() { return !this.#first && !this.back; };
...
const iterator = new ChainIterator(head);
...
for (const node of iterator) {
  console.log(iterator.prev().value);
}
*/